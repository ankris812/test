...
function loadpage(){
  ...
  if(page=="admin"){
    ...
    for(var key in db.pages){
      if(db.pages[key].type=="blog"){
        blogContent+="<a href='https://"+db.user+".github.io/"+db.repo+"/?"+key+"'><div class='blog-container'><p class='blog-title'>"+db.pages[key].title+"</p><p class='blog-date'>"+db.pages[key].date+"</p><p class='blog-content'>"+db.pages[key].content+"</p></div></a>";
      }
    }
    for(var i=0;i<c.length;i++){
